...
const SIMPLE_PERMISSION_MAP: Record<string, string> = {
 ...
 */
function resolveMediaPermissions(permissions: string[]): string[] {
  ...
  const perms = new Set<string>(MEDIA_BASE_PERMISSIONS); // always include legacy
...
function hasMediaPermission(permissions: string[]): boolean {
...
const withPermissionKit: ConfigPlugin<PermissionKitPluginProps> = (
  ...
  const hasAnyAndroidPermission = permissions.some(
    (p) => p in SIMPLE_PERMISSION_MAP || p === 'location' || p === 'media' || p.startsWith('media:')
  );
  ...
  if (hasAnyAndroidPermission) {
    config = withAndroidManifest(config, (config) => {
      ...
      const existingPermissions = usesPermission.map(
